  ...
  if (pj64.romInfo != null) {
    ...
    if (rando != 0) {
      ...
      oot = mem.bindvars({ready:true,currentMode:oot.currentMode}, [
        ...
        [ coop + 4, 'player_id', u8 ],
        // [ coop + 5, 'player_name_id', u8 ],
        [ coop + 6, 'incoming_player', u16 ],
        [ coop + 8, 'incoming_item', u16 ],
        // [ coop + 10, 'mw_send_own_items', u8 ],
      ...
      if (mem.u16[0x801C8544] == 0x004F && !defeatedGanon) {
        for (var actor = mem.u32[0x801CA11C]; actor; actor = mem.u32[actor+0x124]) {
          if (mem.u16[actor] == 0x017A && mem.s8[actor+0xAF] <= 0) {
            defeatedGanon = true;
            if (connection) connection.write(new Buffer([0x05, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]));
          }
        }
      }
    }
